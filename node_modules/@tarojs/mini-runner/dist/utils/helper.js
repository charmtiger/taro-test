...
function getTaroJsQuickAppComponentsPath(nodeModulesPath) {
    const taroJsQuickAppComponentsPkg = runner_utils_1.getInstalledNpmPkgPath(runner_utils_1.taroJsQuickAppComponents, nodeModulesPath);
...
function getImportTaroSelfComponents(filePath, nodeModulesPath, outputDir, taroSelfComponents) {
    ...
    taroSelfComponents.forEach(c => {
        ...
        const cRelativePath = runner_utils_1.promoteRelativePath(path.relative(filePath, cMainPath.replace(nodeModulesPath, path.join(outputDir, 'npm'))));
...
function generateQuickAppUx({ script, template, style, imports }) {
    ...
    if (style) {
        if (runner_utils_1.REG_STYLE.test(style)) {
    ...
    if (script) {
        if (runner_utils_1.REG_SCRIPT.test(script)) {
...
function generateQuickAppManifest({ appConfig, quickappJSON, pageConfigs, designWidth }) {
    ...
    pages.forEach(element => {
        ...
        routerPages[runner_utils_1.removeHeadSlash(path.dirname(element))] = pageConf;
    });
    delete quickappJSON.customPageConfig;
    const routerEntry = pages.shift();
    const router = {
        entry: runner_utils_1.removeHeadSlash(path.dirname(routerEntry)),
        pages: routerPages
    };
    // 生成 display
    ...
    pageConfigs.forEach((item, page) => {
        if (item) {
            display.pages[runner_utils_1.removeHeadSlash(path.dirname(page))] = item;
        }
    });
