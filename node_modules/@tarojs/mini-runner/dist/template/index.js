...
function buildAttribute(attrs, nodeName) {
    function getValue(key) {
        if (adapters_1.Adapter.type === "swan" /* SWAN */ && shared_1.isArray(swanSpecialAttrs[nodeName]) && swanSpecialAttrs[nodeName].includes(key)) {
            return `= ${attrs[key]} =`;
        }
        return `{ ${attrs[key]} }`;
    }
...
const dataKeymap = (keymap) => {
    return adapters_1.Adapter.type === "swan" /* SWAN */ ? `{ ${keymap} }` : keymap;
};
function buildStandardComponentTemplate(comp, level, supportRecursive) {
    const nextLevel = supportRecursive ? 0 : level + 1;
    const child = adapters_1.Adapter.type === "swan" /* SWAN */ && comp.nodeName === 'text'
...
function buildXsTemplate() {
    let xs = '';
    if (adapters_1.Adapter.type === "weapp" /* WEAPP */ || adapters_1.Adapter.type === "qq" /* QQ */) {
        xs = `<wxs module="xs" src="./utils.${adapters_1.Adapter.xs}" />`;
    }
    else if (adapters_1.Adapter.type === "alipay" /* ALIPAY */) {
        xs = `<import-sjs name="xs" from="./utils.${adapters_1.Adapter.xs}" />`;
    }
    else if (adapters_1.Adapter.type === "swan" /* SWAN */) {
        xs = `<import-sjs module="xs" src="./utils.${adapters_1.Adapter.xs}" />`;
    }
    return xs;
}
function buildXScript() {
    const exportExpr = adapters_1.Adapter.type === "alipay" /* ALIPAY */ ? 'export default' : 'module.exports =';
    return `${exportExpr} {
  c: function(i, prefix) {
...
function buildComponentTemplate(comp, level, supportRecursive) {
...
function buildFocusComponentTemplte(comp, level) {
    const attrs = Object.assign({}, comp.attributes);
    const templateName = adapters_1.supportXS()
...
function buildTemplate(level, supportRecursive, restart = false) {
    const miniComponents = shared_1.createMiniComponents(shared_1.internalComponents, adapters_1.Adapter.type === "alipay" /* ALIPAY */);
    const components = Object.keys(miniComponents).filter(c => component_1.componentConfig.includes.size ? component_1.componentConfig.includes.has(c) : true);
    let template = '';
...
function buildThirdPartyAttr(attrs) {
    return [...attrs].reduce((str, attr) => {
        if (attr.startsWith('@')) { // vue event
            if (adapters_1.Adapter.type === "alipay" /* ALIPAY */) {
...
function buildBaseTemplate(maxLevel, supportRecursive) {
