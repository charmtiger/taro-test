    ...
    constructor(hostRef) {
        ...
        this.getHeightByIndex = () => {
            const height = this.index.map(i => {
                let factor = 0;
                if (this.mode === 'time') {
                    factor = constant.LINE_HEIGHT * 4;
                }
                return constant.TOP - constant.LINE_HEIGHT * i - factor;
            });
            return height;
        };
        // 隐藏 picker
        ...
        this.handleChange = () => {
            this.hidePicker();
            this.index = this.height.map(h => (constant.TOP - h) / constant.LINE_HEIGHT);
        ...
        this.updateHeight = (height, columnId, needRevise = false) => {
            ...
            if (needRevise) {
                ...
                if (!compareTime(start, end))
                    return;
                const range = [hoursRange.slice(), minutesRange.slice()];
                const timeList = this.height.map(h => (constant.TOP - h) / constant.LINE_HEIGHT);
                const timeStr = timeList.map((n, i) => range[i][n]).join(':');
                if (!compareTime(start, timeStr)) {
                    // 修正到 start
                    const height = start
                        .split(':')
                        .map(i => constant.TOP - constant.LINE_HEIGHT * (+i + 4));
        ...
        this.handleColumnChange = (height, columnId) => {
            this.onColumnChange.emit({
                ...
                value: (constant.TOP - height) / constant.LINE_HEIGHT
            });
        };
        this.updateDay = (value, fields) => {
            ...
            if (fields === 0) {
                ...
                const height = constant.TOP - constant.LINE_HEIGHT * index;
