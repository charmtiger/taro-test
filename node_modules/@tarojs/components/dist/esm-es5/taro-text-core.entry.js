import { r as registerInstance, h, H as Host } from './core-42efd82a.js';
import { c as classNames } from './index-be61bb74.js';
var Text = /** @class */ (function () {
    function Text(hostRef) {
        registerInstance(this, hostRef);
        this.selectable = false;
    }
    Text.prototype.render = function () {
        var cls = classNames('taro-text', {
            'taro-text__selectable': this.selectable
        });
        return (h(Host, { class: cls }));
    };
    Object.defineProperty(Text, "style", {
        get: function () { return ".taro-text{-moz-user-select:none;-webkit-user-select:none;-ms-user-select:none;user-select:none;display:inline}.taro-text__selectable{-moz-user-select:text;-webkit-user-select:text;-ms-user-select:text;user-select:text}"; },
        enumerable: true,
        configurable: true
    });
    return Text;
}());
export { Text as taro_text_core };
